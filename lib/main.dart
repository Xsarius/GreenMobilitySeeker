...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _startBackgroundFetch() {
    BackgroundFetch.configure(
      ...
      List<Map<String, dynamic>> cars = await getCarList();
      int carCount = cars.length;

      if (carCount > 0) {
        await _showNotification(
          carCount == 1
              ? 'There is $carCount GreenMobility nearby that can be charged'
              : 'There are $carCount GreenMobilities nearby that can be charged',
        );
      }
                ...
                child: Column(
                  children: <Widget>[
                    Padding(
                      ...
                      child: Text(
                        _carCount == 0
                            ? 'There are no cars nearby that can be charged'
                            : _carCount == 1
                                ? 'There is $_carCount GreenMobility nearby that can be charged'
